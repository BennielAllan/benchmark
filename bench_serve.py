...
def sample_random_requests(
    ...
    input_requests = []
    #prompt="a "*int(input_len)
    #actual_len=len(tokenizer.tokenize(prompt))
    for i in range(num_prompts):
        prompt = tokenizer.decode([(random.randint(0,16384)) % tokenizer.vocab_size
                                    for j in range(input_len)])
        actual_len=len(tokenizer.tokenize(prompt))
        print(actual_len)
        input_requests.append((prompt, actual_len,
                               int(output_len), None))
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        ...
        f.write("tokens:"+str(output_len)+"len itl"+str(len(outputs[i].itl)))
        f.write("\n")
